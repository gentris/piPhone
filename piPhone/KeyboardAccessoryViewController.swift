...
final class KeyboardAccessoryViewController: UIInputViewController {
    ...
    override func viewDidLoad() {
        ...
        let bar = KeyboardAccessoryView { [weak self] token in
            guard let self else { return }

            switch token {
            case "ctrl":
                self.keysDelegate?.didClickControlKey(key: self.ctrlKey)
            case "esc":
                self.keysDelegate?.didClickSpecialKey(key: self.escKey)
            case "tab":
                self.keysDelegate?.didClickSpecialKey(key: self.tabKey)
            case "up":
                self.keysDelegate?.didClickSpecialKey(key: self.upKey)
            case "down":
                self.keysDelegate?.didClickSpecialKey(key: self.downKey)
            case "left":
                self.keysDelegate?.didClickSpecialKey(key: self.leftKey)
            case "right":
                self.keysDelegate?.didClickSpecialKey(key: self.rightKey)
            default:
                break
            }
        }
