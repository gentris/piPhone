...
class TerminalViewController: UIViewController {
    ...
    private func loadSubViews() {
        ...
        self.keyboardInputWebView = KeyboardInputWebView(
            frame: .zero, configuration: configuration, specialKeysDelegate: self)

        self.terminalWebView = TerminalWebView(
            frame: self.getTerminalWebViewFrame(), configuration: configuration)
        self.terminalWebView.addInteraction(
            TermGesturesInteraction(
                jsScrollerPath: "t.scrollPort_.scroller_",
                keyboardInputWebView: self.keyboardInputWebView))
    ...
    func userContentController(
        ...
        if message.name == self.terminalWebViewScriptName {
            ...
            if operation == "terminalReady" {
                ...
                let data =
                    "{\"cols\": \(self.terminalWebView.cols), \"rows\": \(self.terminalWebView.rows)}"
                self.peripheral?.write(
                    data: data, characteristic: self.peripheral?.screenCharacteristic)
            }
            ...
            if op == "out" {
                ...
                self.peripheral?.write(
                    data: data, characteristic: self.peripheral?.commandCharacteristic)
...
extension TerminalViewController: SpecialKeysDelegate {
    func didClickSpecialKey(key: Key) {
        self.peripheral?.write(
            data: key.ansi, characteristic: self.peripheral?.commandCharacteristic)
    }
