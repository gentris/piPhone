...
class BluetoothManager: NSObject, ObservableObject {
    ...
    private func initScanTimer() -> Timer {
        return Timer.scheduledTimer(
            timeInterval: 10, target: self, selector: #selector(startScanning), userInfo: nil,
            repeats: true)
    }

    private func initPauseTimer() -> Timer {
        return Timer.scheduledTimer(
            timeInterval: 2, target: self, selector: #selector(stopScanning), userInfo: nil,
            repeats: true)
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        for service in services {
            ...
            peripheral.discoverCharacteristics(
                [screenCharacteristicUUID, commandCharacteristicUUID], for: service)
        }
    }
